...
PRECIP_THRESHOLDS_URL = "https://umbrales-precipitacion-api.scgarzonp.workers.dev/api/estaciones"
...
def fetch_recent_levels(station_codigo: str, days: int = 30) -> list[dict]:
    """Fetch recent river level readings for a station."""
    since = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%dT%H:%M:%SZ")
    readings = []
    url = f"{BASE_URL}/estaciones/{station_codigo}/nivel/?calidad=1"
    while url:
        try:
            ...
            # Stop paginating when we reach data older than our window
...
def fetch_recent_precipitation(station_codigo: str, days: int = 30) -> list[dict]:
    """Fetch recent precipitation readings for a station."""
    since = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%dT%H:%M:%SZ")
    readings = []
    url = f"{BASE_URL}/estaciones/{station_codigo}/precipitacion/?calidad=1"
    while url:
        try:
            response = requests.get(url, timeout=15)
            response.raise_for_status()
